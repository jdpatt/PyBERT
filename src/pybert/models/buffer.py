...
class Buffer(HasTraits):
    ...
    def _ibis_filepath_changed(self, new_filepath):
        ...
        self.status = f"Parsing IBIS file: {new_filepath}"
        try:
            ibis = IBISModel(new_filepath)
            self.use_ibis = True
            self.ibis_model = ibis
        except Exception as err:
            self.status = "IBIS file parsing error!"
            self.log(f"Failed to open and/or parse IBIS file!\n{err}", alert=True, exception=err)
            self.use_ibis = False
            self.ibis_filepath = ""
            self.ibis_model = None

    def _ami_filepath_changed(self, ami_filepath):
        self.status = f"Parsing AMI file, {ami_filepath}"
        try:
            ...
            self.log(f"Failed to open and/or parse AMI file!\n{err}", alert=True)
    ...
    def _dll_filepath_changed(self, new_value):
        try:
            ...
            self.log(f"Failed to open DLL/SO file!\n{err}", alert=True)
