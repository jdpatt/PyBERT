...
from pyibisami.ami.model import AMIModel, AMIModelInitializer, AMIParamConfigurator
...
def run_ami_model(
    model: AMIModel,
    param_cfg: AMIParamConfigurator,
    use_getwave: bool,
    ui: float,
    ts: float,
    chnl_h: Rvec,
    x: Rvec,
    bits_per_call: int = 0,  # noqa: F405
) -> tuple[Rvec, Rvec, Rvec, Rvec, str, list[str]]:  # noqa: F405
    """
    Run a simulation of an IBIS-AMI model.

    Args:
        ami_model: An ``AMIModel`` instance.
    ...
    msg = "\n".join(
        [  # Python equivalent of Haskell's `unlines()`.
    ...
    try:
        ...
        logger.error(f"out_h shapes: {[h.shape for h in out_h]}")
        raise
    clks = array([])
