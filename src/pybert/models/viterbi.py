...
from pybert.constants import TWOPI, Rmat, Rvec
from pybert.utility.functional import fst
from pybert.utility.math import all_combs
...
class ViterbiDecoder(ABC, Generic[S, X]):
    ...
    def step_trellis(self, x: X, priming: bool = False) -> int:
        ...
        for r in range(num_states):
            new_probs = np.array(
                [
                    0 if self.trans[r][s] == 0 else trellis[-1][r][0] * self.trans[r][s] * self.prob(s, x)
